...
public class FactorialTest {
	...
	public void setup() {
		...
		sixMinusOne = new Application(new Application(new Variable("-"),
				new Constant(6)), new Constant(1));
		fFive = new Application(new Variable("f"), new Variable("five"));
		letVal = new LetVal("five", sixMinusOne, fFive);
		letFun = new LetFun("f", "n", fBody, fFive);
		nestedLets = new LetFun("f", "n", fBody, new LetVal("five",
				sixMinusOne, fFive));
		env0 = InitialEnvironment.getEnvironment();
		env1 = new ActivationRecord("f", "n", fBody, env0);
		env2 = new ActivationRecord("five", sixMinusOne.eval(env1), env1);
		env3 = new ActivationRecord("five", sixMinusOne.eval(env0), env0);
	}

	/**
	 * When this test is run, it does the equivalent of the following ML
	 * program, which calculates 120:
	 * 
	 * <pre>
	...
	public void letValToString() {
		assertEquals("let val five = ((- 6) 1) in (f five) end",
				letVal.toString());
	}
	...
	public void nestedLetsInEnv0() {
		assertEquals(Integer.valueOf(120),
				nestedLets.eval(env0).as(Integer.class));
	}
...
//	}
	...
	private Expression getBody() {
		...
		Expression nMinusOne = new Application(new Application(minus, n), one);
		Expression fNMinusOne = new Application(f, nMinusOne);
		Expression product = new Application(
				new Application(times, fNMinusOne), n);
		Expression nEqualsZero = new Application(new Application(equals, n),
				zero);
